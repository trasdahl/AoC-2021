...
# For å effektivisere kan vi først finne minste x_init som kan gi treff
xmin = x1 + 1
for y_init in range(abs(y0)+1):
    for x_init in range(y_init - 5, y_init + 5):
        x, y = throw(x_init, y_init, 1000)
        if target_hit(x, y, target) and x_init < xmin:
            xmin = x_init
                
print("Smallest x_init to hit target:", xmin)

for x_init in range(xmin, x1+1):
    for y_init in range(y0, abs(y0)+1):
        x, y = throw(x_init, y_init, 1000)
        if target_hit(x, y, target):
            hits += 1
            y_rec = y.max()
            successfull_inits += [(x_init, y_init)]
        # print(f"y_init={y_init}. Hitting target? {target_hit_vertically(x, y, target)}. Highpoint = {y.max()}")

print("Successful init velocities:\n", successfull_inits)
print("Hits:", hits)
print("Highest throw:", y_rec)
